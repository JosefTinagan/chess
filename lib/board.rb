class Board
	...
	def queen_move(start_pos,end_pos)
		x = rook_move(start_pos,end_pos)
		y = bishop_move(start_pos,end_pos)
		return x || y
	end

	def king_move(start_pos,end_pos)
		valid_moves = []
		a,b = start_pos[0], start_pos[1]
		tmp = get_cell(a,b)
		color = tmp.color

		valid_moves.push([a + 1, b + 1]).push([a - 1, b - 1]).push([a + 1, b - 1])
		.push([a - 1, b + 1]).push([a + 1, b]).push([a - 1, b]).push([a, b + 1])
		.push([a, b - 1])
		valid_moves.select!{ |x| x[0] >= 0 && x[0] <= 7 && x[1] >= 0 && x[1] <= 7 }
		tmp_arr = []
